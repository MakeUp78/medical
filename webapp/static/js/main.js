...
async function findBestFrontalFrame() {
  ...
  updateStatus('🔍 Ricerca frame ottimale con sistema unificato...');

  const frameSource = new FrameSource(video, 'video');
  const processor = new UnifiedFrameProcessor();

  await processor.processStream(frameSource, {
    stepSize: 0.33,
    onProgress: (frameData) => {
      const data = {
        frameIndex: Math.floor(frameData.time / 0.33),
        time: frameData.time,
        score: frameData.result.frontality_score,
        pose: frameData.result.pose_angles,
        landmarks: frameData.result.landmarks,
    ...
    onComplete: (topFrames) => {
      if (topFrames.length > 0) {
        const best = topFrames[0];
        highlightBestFramesInTable(topFrames.map(f => ({
          time: f.time,
          score: f.result.frontality_score,
...
async function startAutomaticVideoScanning() {
  ...
  const video = window.currentVideo;
  clearDebugAnalysisTable();
  updateStatus('🤖 Scansione automatica con sistema unificato...');
  ...
  await processor.processStream(frameSource, {
    ...
    onProgress: (frameData) => {
      const data = {
        frameIndex: Math.floor(frameData.time / 0.5),
        time: frameData.time,
        score: frameData.result.frontality_score,
        pose: frameData.result.pose_angles,
        landmarks: frameData.result.landmarks,
