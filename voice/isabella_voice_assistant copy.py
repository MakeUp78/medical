...
Usage:
    ...
    # ESEMPI RIMOSSI - tutto ora viene dal JSON isabella_voice_config.json
    # I messaggi e i comandi vengono caricati automaticamente dalla configurazione
...
class IsabellaVoiceAssistant:
    ...
    def __init__(self, config_file: str = "voice_config.json"):
        ...
        # Handler di fallback per comandi non riconosciuti
        self.query_fallback_handler = None
    ...
    def set_query_fallback_handler(self, handler):
        """Imposta handler di fallback per query intelligenti"""
        self.query_fallback_handler = handler
        logger.info("🧠 Handler fallback query intelligenti registrato")
    ...
    def _load_default_commands(self):
        """Comandi predefiniti DISABILITATI - ora tutto viene dal JSON"""
        # NOTA: Tutti i comandi (inclusi quelli di sistema) sono ora nel file
        # isabella_voice_config.json per controllo completo e centralizzato
        logger.info("🚫 Comandi hardcoded disabilitati - usando solo JSON")

    def _load_json_commands(self):
        """Carica comandi di sistema dal file JSON"""
        try:
            commands = self.config.get("commands", [])
            logger.info(f"📋 Caricamento {len(commands)} comandi dal JSON")
            
            for cmd_config in commands:
                if not cmd_config.get("enabled", True):
                    continue
                    
                action_type = cmd_config.get("action", {}).get("type", "system")
                
                # Carica solo comandi di sistema nell'assistente base
                if action_type == "system":
                    patterns = cmd_config.get("patterns", [])
                    action_name = cmd_config.get("name", "unnamed")
                    function_name = cmd_config.get("action", {}).get("function")
                    confirmation = cmd_config.get("confirmation", "Comando eseguito")
                    
                    # Registra comando di sistema
                    if function_name and hasattr(self, f"_handle_{function_name}"):
                        handler = getattr(self, f"_handle_{function_name}")
                        self.add_command(
                            keywords=patterns,
                            action=action_name,
                            handler=handler,
                        ...
                        logger.info(f"✅ Comando sistema registrato: '{patterns[0]}' -> {function_name}")
                    else:
                        logger.debug(f"⏭️ Comando GUI ignorato: '{action_name}' (sarà gestito dall'integrazione)")
                        
        except Exception as e:
            logger.error(f"❌ Errore caricamento comandi JSON: {e}")
    ...
    async def _handle_show_help(self, text: str, command: VoiceCommand):
        """Gestisce comando aiuto vocale"""
        await self._handle_help(text, command)

    async def _handle_show_system_help(self, text: str, command: VoiceCommand):
        """Gestisce comando aiuto sistema"""
        await self.speak("Comandi disponibili: aiuto, stato, esci, analizza volto, carica immagine, e molti altri.")

    async def _handle_show_system_status(self, text: str, command: VoiceCommand):
        """Gestisce comando stato sistema"""
        duration = int(time.time() - self.session_start)
        await self.speak(f"Sistema attivo da {duration} secondi. Eseguiti {self.command_count} comandi.")

    async def _handle_exit_assistant(self, text: str, command: VoiceCommand):
        """Gestisce comando uscita"""
        await self.speak("Assistente vocale disattivato. Arrivederci!")
        self.is_active = False

    async def _handle_mute_voice(self, text: str, command: VoiceCommand):
        """Gestisce comando silenzia"""
        # Questo sarà gestito dall'integrazione GUI
        logger.info("🔇 Comando mute ricevuto")

    async def _handle_unmute_voice(self, text: str, command: VoiceCommand):
        """Gestisce comando riattiva voce"""
        await self.speak("Voce riattivata")
        logger.info("🔊 Comando unmute ricevuto")
    ...
    def start_listening(self, activate_immediately=False):
        """Avvia microfono in ascolto per activation keywords"""
        ...
        # MICROFONO SEMPRE ATTIVO, ASSISTENTE NO
        self.is_active = False  # Assistente disattivato fino a keywords
        logger.info("🎤 Microfono attivato - In attesa di activation keywords")
        ...
        logger.info("👂 Microfono in ascolto per activation keywords")
    ...
    def _listen_continuously(self):
        """Loop continuo di ascolto - gestisce attivazione e comandi"""
        ...
        while self.is_listening:
            try:
                ...
                # Log sempre per debugging attivazione
                logger.info(f"🎤 Testo riconosciuto: '{text}' (Active: {self.is_active})")
    ...
    def _check_activation(self, text: str) -> bool:
        ...
        # Log per debug
        logger.info(f"🔍 Checking activation for: '{text_normalized}'")
        logger.info(f"🔍 Keywords: {activation_keywords}")
        
        # Controllo diretto con keywords esatte
        for keyword in activation_keywords:
            keyword_normalized = keyword.lower().strip()
            if keyword_normalized in text_normalized:
                logger.info(f"✅ ACTIVATION FOUND: '{keyword_normalized}' in '{text_normalized}'")
                return True
        
        # Controllo con varianti di "simmètra" che Google Speech riconosce male
        simmetra_variants = [
            "simme", "simmet", "simmetra", "simmètra", "symmetra", "simetra", 
            "si metra", "ci me", "theme", "semi", "same"
        ]
        
        # Controllo se c'è un saluto + variante di simmètra
        starters = ["hey", "ciao", "ehi", "hi", "ey", "ay"]
        
        # Cerca starter + variante
        for starter in starters:
            if starter in text_normalized:
                for variant in simmetra_variants:
                    if variant in text_normalized:
                        logger.info(f"✅ ACTIVATION FOUND: '{starter}' + '{variant}' in '{text_normalized}'")
                        return True
        
        # Controllo se c'è solo una variante di simmètra (senza saluto)
        for variant in simmetra_variants:
            if variant in text_normalized and len(text_normalized.replace(variant, "").strip()) <= 3:
                logger.info(f"✅ ACTIVATION FOUND: solo '{variant}' in '{text_normalized}'")
                return True
        
        logger.debug(f"❌ No activation found in: '{text_normalized}'")
        return False

    async def _handle_activation(self, text: str):
        """Gestisce attivazione assistente tramite activation keywords"""
        # Double check - se già attivo, ignora
        if self.is_active:
            logger.debug("🔄 Assistente già attivo, ignorando riattivazione")
            return
        ...
        logger.info(f"🎯 Assistente ATTIVATO con: '{text}'")

        # Feedback vocale BREVE
        try:
            await self.speak_message("activation")
        except Exception as e:
            logger.error(f"❌ Errore feedback attivazione: {e}")

        # Timer disattivazione automatica
        timeout = self.config.get("advanced_settings", {}).get("auto_deactivate_timeout", 10)
        if timeout > 0:
            threading.Timer(timeout, self._auto_deactivate).start()
            logger.debug(f"⏰ Timer disattivazione impostato: {timeout}s")

    def _auto_deactivate(self):
        """Disattivazione automatica silenziosa dopo timeout"""
        if self.is_active:
            self.is_active = False
            logger.info("⏰ Assistente disattivato automaticamente - in attesa di nuove keywords")
            # Nessun messaggio vocale per evitare interferenze
    ...
    async def _process_command(self, text: str):
        ...
        if not command_found:
            # Prova a processare come query intelligente se disponibile
            fallback_handled = False
            if hasattr(self, 'query_fallback_handler') and self.query_fallback_handler:
                try:
                    fallback_handled = await self.query_fallback_handler(text)
                except Exception as e:
                    logger.error(f"❌ Errore fallback query: {e}")
            
            # Se non gestito dal fallback, usa messaggio standard
    ...
    async def speak_startup(self, app_name: str = None, custom_message: str = None):
        """Messaggio di avvio dal JSON - NO messaggi hardcoded"""
        if custom_message:
            await self.speak(custom_message)
        else:
            # Usa SEMPRE i messaggi dal JSON
            await self.speak_message("welcome_user")
...
async def example_usage():
    ...
    # DEMO DISABILITATA - tutto ora viene dal JSON
    # Messagi e comandi vengono caricati da isabella_voice_config.json
